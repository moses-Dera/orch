...
@app.command()
def init():
    """Initialize Orch in the current repository: detect remote, register repo, install pre-commit hook."""
    import subprocess
    # 1. Detect git remote
    try:
        res = subprocess.run(["git", "remote", "get-url", "origin"], capture_output=True, text=True)
        repo_url = res.stdout.strip() if res.returncode == 0 else None
        if not repo_url:
            # Try to get any remote
            res = subprocess.run(["git", "remote"], capture_output=True, text=True)
            remotes = res.stdout.strip().splitlines() if res.returncode == 0 else []
            if remotes:
                res = subprocess.run(["git", "remote", "get-url", remotes[0]], capture_output=True, text=True)
                repo_url = res.stdout.strip() if res.returncode == 0 else None
    except Exception:
        repo_url = None

    if not repo_url:
        console.print("[bold red]Error:[/bold red] Not a git repository, or no git remotes configured.")
        console.print("[dim]Please initialize git and add a remote origin first.[/dim]")
        raise typer.Exit(1)

    # 2. Register repo with backend
    try:
        with console.status("[bold green]Registering repository with Orch...[/bold green]"):
            r = httpx.post(
                f"{API_URL}/api/v1/onboarding/register-repo-cli",
                json={"repo_url": repo_url},
                headers=headers(),
                timeout=15.0
            )
            if r.status_code == 401:
                console.print("[bold red]Authentication failed.[/bold red] Run `orch login` first.")
                raise typer.Exit(1)
            r.raise_for_status()
            data = r.json()
    except httpx.HTTPStatusError as e:
        console.print(f"[bold red]Registration failed:[/bold red] {e.response.json().get('detail', {}).get('message', str(e))}")
        raise typer.Exit(1)
    except Exception as e:
        console.print(f"[bold red]Failed to connect to Orch server:[/bold red] {e}")
        raise typer.Exit(1)

    org_name = data.get("org_name", "your org")
    console.print(f"[bold green]Repository registered[/bold green] successfully under [bold]{org_name}[/bold].")
    ...
    if not git_dir.exists():
        console.print("[bold yellow]Warning:[/bold yellow] Could not locate .git directory. Skipping hook installation.")
        console.print("To manually install, add `orch pre-commit` to your git pre-commit hook.")
        raise typer.Exit(0)
...
# Orch pre-commit hook
# Generated by orch init

if command -v orch >/dev/null 2>&1; then
  orch pre-commit
else
  echo "[Orch] warning: orch command not found, skipping check."
fi
""")
        hook_path.chmod(0o755)
        console.print(f"[bold green]Git pre-commit hook installed[/bold green] at {hook_path}")
    except Exception as e:
        console.print(f"[bold yellow]Warning:[/bold yellow] Failed to install pre-commit hook: {e}")

    console.print("[bold green]Orch initialization complete![/bold green]")


@app.command(hidden=True)
def pre_commit():
    """Internal command called by the git pre-commit hook."""
    import subprocess
    try:
        res = subprocess.run(
        ...
        console.print(f"[bold red]Failed to get staged files:[/bold red] {e}")
        raise typer.Exit(1)

    if not files:
        raise typer.Exit(0)

    console.print("[bold cyan]Orch: Auditing staged changes...[/bold cyan]")
    ...
    for file_path in files:
        try:
            ...
            console.print(f"[bold red]Failed to get diff for {file_path}:[/bold red] {e}")
            continue
        ...
        try:
            ...
            console.print(f"[bold yellow]Skipping {file_path}: review request failed[/bold yellow] ({e})")
            continue

        issues = data.get("issues", [])
        if not data.get("clean", True) and issues:
            console.print(f"\n[bold yellow]Findings in {file_path}:[/bold yellow]")
            for issue in issues:
                severity = issue.get("severity", "info").lower()
                color = "red" if severity == "critical" else "yellow" if severity == "warning" else "cyan"
                line_info = f"L{issue.get('line')}: " if issue.get("line") is not None else ""
                console.print(f"  [{color}][{severity.upper()}][/{color}] {line_info}{issue.get('title')}")
                console.print(f"    {issue.get('detail')}")
                if issue.get("suggested_fix"):
                    console.print(f"    [dim]Suggested fix: {issue.get('suggested_fix')}[/dim]")

                if severity == "critical":
                    has_critical = True

    if has_critical:
        console.print("\n[bold red]Commit blocked by Orch: Critical constraints violated.[/bold red]")
        raise typer.Exit(1)
    else:
        console.print("\n[bold green]Orch: Check passed.[/bold green]")
        raise typer.Exit(0)
